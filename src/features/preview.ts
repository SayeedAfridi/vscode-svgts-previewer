...
export class Preview {
    ...
    this._panel.webview.onDidReceiveMessage((message) => {
      ...
      if (message.command === "changeBoundingBoxVisibility") {
        vscode.workspace
          ...
          .update("preview.boundingBox", message.payload.visible, true);
        this.telemetryReporter.sendTelemetryEvent(
          TELEMETRY_EVENT_TOGGLE_BOUNDING_BOX,
      ...
      if (message.command === "changeTransparencyGridVisibility") {
        vscode.workspace
          .getConfiguration("svg")
          .update("preview.transparencyGrid", message.payload.visible, true);
        this.telemetryReporter.sendTelemetryEvent(
          TELEMETRY_EVENT_TOGGLE_TRANSPARENCY_GRID,
          { visible: message.payload.visible }
        );
  ...
  public async update(resource?: vscode.Uri, isSvgr?: boolean) {
    if (resource) {
      this._resource = resource;
    }
    this._panel.title = Preview.getPreviewTitle(this._resource.fsPath);

    const message = await this.getUpdateWebViewMessage(this._resource, isSvgr);
  ...
  private async getUpdateWebViewMessage(uri: vscode.Uri, isSvgr?: boolean) {
    ...
    const showTransparencyGrid = <boolean>(
      vscode.workspace.getConfiguration("svg").get("preview.transparencyGrid")
    );
    ...
    if (isSvgr) {
      payload.svg = getOnlySvgString(data);
    }

    return updatePreview(payload);
  }
